...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := grand50_addrdec.G50_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = grand50_addrdec.G50_testnetPrivateWIFCompressed
	}

	wif, _ := grand50_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	grand50_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := grand50_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := grand50_addrdec.G50_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = grand50_addrdec.G50_testnetAddressP2SH
	}
	...
	address, _ := grand50_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := grand50_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
